...
class VM {
    ...
    run(debug) {
        ...
        return co(function * () {
            while (true) {
                if (state.code.isNil()) {
                    if (state.env.cdr().isNil() && state.dump.isNil()) {
                    ...
                    if (inst instanceof App) {
                        ...
                        try {
                            ...
                            throw new RuntimeError([inst.pos], "function out of bounds: " + (inst.func + 1).toString())
                                .addTrace(trace);
                        }
                        try {
                            ...
                            throw new RuntimeError([inst.pos], "argument out of bounds: " + (inst.arg + 1).toString())
                                .addTrace(trace);
                        }
