...
class VM {
    constructor(code, input, output) {
        ...
        const OUT = new Prim("Out", x => {
                if (x instanceof Char) {
                    output(String.fromCharCode(x.code));
                    return x;
                }
                else {
    ...
    run() {
        ...
        return co(function * () {
            while (true) {
                if (state.code.isNil() && state.env.cdr().isNil() && state.dump.isNil()) {
                    return state.env.car();
                }
                if (state.code.isNil() && !state.env.isNil() && !state.dump.isNil()) {
                    let f = state.env.car();
                    let d = state.dump.car();
                    state.code = d.fst();
                    state.env  = d.snd().cons(f);
                    state.dump = state.dump.cdr();
                }
                else {
                    let inst = state.code.car();
                    if (inst instanceof App) {
                        let ff = state.env.at(inst.func);
                        let fa = state.env.at(inst.arg);
